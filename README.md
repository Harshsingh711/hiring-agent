...
Run a local site where you upload a PDF and get the same rubric scores, evidence, and improvement notes.

```bash
$ python -m uvicorn app:app --host 127.0.0.1 --port 8000
```

Open [http://127.0.0.1:8000](http://127.0.0.1:8000). Scoring can take several minutes because the pipeline parses the PDF, fetches GitHub if a profile is listed, then evaluates with the configured model.
...
1. If development mode is on, the PDF extraction result is cached to `cache/resumecache_<content-hash>.json`.
2. If a GitHub profile is found in the resume, repositories are fetched and cached to `cache/githubcache_<content-hash>.json`.
